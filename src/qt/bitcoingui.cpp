...
void BitcoinGUI::createActions()
{
    ...
    openWebsiteAction = new QAction(QIcon(":/icons/website"), tr("&Website"), this);
    openWebsiteAction->setToolTip(tr("Official Website"));
    openWebsiteAction->setStatusTip(openWebsiteAction->toolTip());
    openWebsiteAction->setCheckable(true);
    openWebsiteAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_W));
    ...
    aboutAction = new QAction(QIcon(":/icons/coin"), tr("&About VitalCoin"), this);
...
//    trayIconMenu->addSeparator();
//    trayIconMenu->addAction(openBrowserAction);
//    trayIconMenu->addAction(openWebsiteAction);
    trayIconMenu->addSeparator();
...
{
    const QUrl url("http://150.107.3.177/");
...
void BitcoinGUI::openWebsiteClicked()
{
    const QUrl url("http:///");
    QDesktopServices::openUrl(url);

    openWebsiteAction->setChecked(false);
    switch (nCurrentWidget) {
    case 1:
    default:
        gotoOverviewPage();
